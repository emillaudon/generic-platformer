...
use crate::{GameState, ascii::{spawn_ascii_sprite, AsciiSheet}, fadeout::create_fadeout};
...
impl Plugin for CombatPlugin {
    fn build(&self, app: &mut App) {
        app
            .add_system_set(SystemSet::on_update(GameState::Combat).with_system(test_exit_combat).with_system(combat_camera))
...
fn spawn_enemy(mut commands: Commands, ascii: Res<AsciiSheet>) {
    let sprite = spawn_ascii_sprite(&mut commands, &ascii, 'b' as usize, Color::rgb(0.8, 0.8, 0.8,),
...
fn test_exit_combat(mut commands: Commands, mut keyboard: ResMut<Input<KeyCode>>, mut state: ResMut<State<GameState>>, ascii: Res<AsciiSheet>) {
